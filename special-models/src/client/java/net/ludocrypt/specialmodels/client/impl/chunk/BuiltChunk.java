...
public class BuiltChunk {
	...
	protected double getSquaredCameraDistance() {
		double x = this.boundingBox.minX + 8.0 - specialChunkBuilder.client().gameRenderer.getCamera().getPos().x;
		double y = this.boundingBox.minY + 8.0 - specialChunkBuilder.client().gameRenderer.getCamera().getPos().y;
		double z = this.boundingBox.minZ + 8.0 - specialChunkBuilder.client().gameRenderer.getCamera().getPos().z;
		return x * x + y * y + z * z;
	}
	...
	public Task createRebuildTask(ChunkRendererRegionBuilder cache) {
		...
		this.rebuildTask = new RebuildTask(this.getSquaredCameraDistance(), region,
	...
	public class RebuildTask extends Task {
		...
		private RebuildTask.RenderedChunkData render(float cameraX, float cameraY, float cameraZ,
													 SpecialBufferBuilderStorage buffers) {
			RebuildTask.RenderedChunkData renderedChunkData = new RebuildTask.RenderedChunkData();
					...
					if (state.getRenderType() != BlockRenderType.INVISIBLE) {
						...
						if (!models.isEmpty()) {
							...
							for (Pair<SpecialModelRenderer, BakedModel> pair : models) {
								...
								RebuildTask.ReconstructableModel constructedModel = new RebuildTask.ReconstructableModel(model);
								constructedModel
